    ...
    with sync_playwright() as p:
        ...
        for kw_index, kw in enumerate(keywords, start=1):
            ...
            try:
                ...
                # สกัดชื่อหัวตารางภาษาไทยที่มองเห็นจริงบนหน้าจอ
                ...
                for current_page in range(1, total_pages + 1):
                    ...
                    # สกัดเฉพาะแถวตัวยาจริง (ไม่นับหัวตารางหรือแถวกรอง)
                    ...
                    for r in data_rows:
                        ...
                        if len(visible_cells) >= 4:
                            ...
                            # ตรวจสอบว่าช่องแรกเป็นตัวเลขลำดับเท่านั้น
...
if st.button("🚀 เริ่มดึงข้อมูลทั้งหมด", type="primary"):
    if not search_list:
        ...
        df_final = scrape_drug_data(search_list, prog_bar, status_lbl)

        prog_bar.progress(1.0)
        status_lbl.text("✅ ทำการสืบค้นข้อมูลยาครบทุกรายการเรียบร้อยแล้ว!")

        if not df_final.empty:
            st.success(f"ดึงข้อมูลสำเร็จรวมทั้งหมด {len(df_final)} รายการ")
            st.write("ตัวอย่างข้อมูลผลลัพธ์ (10 รายการแรก):")
            st.table(df_final.head(10))

            # บันทึกข้อมูลลง IndexedDB ของอุปกรณ์
            save_to_indexeddb_js(df_final)

            # แปลงไฟล์เพื่อส่งออกเป็น Excel
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
                df_final.to_excel(writer, index=False, sheet_name="FDA_Drug_Results")
            ...
            download_html = f'<p style="margin-top: 5px;"><a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64_excel}" download="fda_combined_drug_results.xlsx" style="color: #1976d2; font-weight: bold; text-decoration: underline;">👉 หากปุ่มด้านบนกดไม่ติด ให้แตะดาวน์โหลดไฟล์ Excel โดยตรงที่นี่</a></p>'
